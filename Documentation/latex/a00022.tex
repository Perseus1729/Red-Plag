...
\begin{DoxyCode}{0}
\DoxyCodeLine{49     \textcolor{keyword}{def }post(request, *args, **kwargs):}
\DoxyCodeLine{50         label=request.POST[\textcolor{stringliteral}{'label'}]}
\DoxyCodeLine{51         username=request.POST[\textcolor{stringliteral}{'username'}]}
\DoxyCodeLine{52         your\_file = request.FILES[\textcolor{stringliteral}{'data'}]}
\DoxyCodeLine{53         add\_data.objects.create( username=username,label=label,data=your\_file)}
...
\DoxyCodeLine{57         \textcolor{keywordflow}{return} JsonResponse(\textcolor{stringliteral}{"{}success"{}},safe=\textcolor{keyword}{False})}
\DoxyCodeLine{58  }
...
\begin{DoxyCode}{0}
\DoxyCodeLine{60     \textcolor{keyword}{def }view\_files(request):}
\DoxyCodeLine{61         data = request.POST}
\DoxyCodeLine{62         your\_files=add\_data.objects.filter(label=data[\textcolor{stringliteral}{'label'}])}
\DoxyCodeLine{63         if(\textcolor{keywordflow}{not} your\_files.exists()):}
\DoxyCodeLine{64             \textcolor{keywordflow}{return} JsonResponse(\textcolor{stringliteral}{"{}You dont have any files"{}},safe=\textcolor{keyword}{False})}
\DoxyCodeLine{65         l=[]}
\DoxyCodeLine{66         users=[]}
\DoxyCodeLine{67         \textcolor{keywordflow}{for} i \textcolor{keywordflow}{in} your\_files:}
\DoxyCodeLine{68             t=tarfile.open(\textcolor{stringliteral}{'media/'}+str(i.data.name),\textcolor{stringliteral}{'r'})}
\DoxyCodeLine{69             L=t.getnames()}
\DoxyCodeLine{70             \textcolor{keywordflow}{for} k \textcolor{keywordflow}{in} L:}
\DoxyCodeLine{71                 l.append(k)}
\DoxyCodeLine{72                 \textcolor{keywordflow}{try}:}
\DoxyCodeLine{73                     users.append(k.split(\textcolor{stringliteral}{'/'})[1])}
\DoxyCodeLine{74                 \textcolor{keywordflow}{except}:}
\DoxyCodeLine{75                     \textcolor{keywordflow}{pass}}
\DoxyCodeLine{76         preprocessing(l,users)}
\DoxyCodeLine{77         data=\{\textcolor{stringliteral}{'png'}:\textcolor{stringliteral}{'result.png'},\textcolor{stringliteral}{'txt'}:\textcolor{stringliteral}{'result.txt'},\textcolor{stringliteral}{'csv'}:\textcolor{stringliteral}{'result.csv'}\}}
\DoxyCodeLine{78         \textcolor{keywordflow}{return} JsonResponse(data, safe=\textcolor{keyword}{False})}
